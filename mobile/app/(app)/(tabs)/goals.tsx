...
export default function Goals() {
  ...
  return (
    <Screen>
      ...
      <Button title="Edit goals & priorities" secondary onPress={()=>router.push('/settings')}/>
      {!profile.goals.length && <Text style={s.body}>Add your first savings goal in Financial setup.</Text>}
      {profile.goals.map((goal) => {
        const progress = Math.min(
          100,
          goal.targetCents===0?100:Math.round((goal.savedCents / goal.targetCents) * 100),
        );
      ...
      <Card tint={colors.soft}>
        ...
        <Text style={s.body}>
          Your goal order sets savings priority. Deadlines and allowed delays are
          part of every purchase check. Saved amounts are reserved within your selected accounts.
        </Text>
      </Card>
    </Screen>
  );
