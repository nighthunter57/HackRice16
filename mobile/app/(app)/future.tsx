...
export default function Future() {
  ...
  const selectedIndex = Math.min(dayIndex, scenario.days.length - 1);
  const selected = scenario.days[selectedIndex];
  const samples = scenario.days;
  const balances = [
    ...analysis.today.days,
  ...
  async function toggleRepair() {
    ...
    try {
      if (offlineDemo) await checkDemo(purchase, !repair);
      else await check(purchase, !repair);
    } catch (cause) {
      setChange(cause instanceof Error ? cause.message : "We couldn’t update this forecast. Please try again.");
      return;
    }
    setChange(
  ...
  return (
    <Screen>
      ...
      >
        {(["today", "wait"] as const).map((value) => (
          ...
          >
            <Text style={s.label}>
              {value === "today"
                ? historical ? `Buy on ${dateLabel(profile.startDate)}` : "Buy today"
                : analysis.wait
                  ? "Wait until safe"
                  : "No purchase"}
            </Text>
      ...
      <Card>
        <Text style={s.heading}>Balance impact</Text>
        <BalanceImpactDetails impact={scenario.balanceImpact} />
        <Text style={s.body}>{balanceImpactExplanation(scenario.balanceImpact)}</Text>
      </Card>
      <Card>
        ...
        <Text style={s.heading}>
          {choice === "today"
            ? historical ? `If you buy on ${dateLabel(profile.startDate)}` : "If you buy today"
            : analysis.wait
        ...
        <View style={s.row}>
          <View style={{ flex: 1 }}>
            <Button
              ...
              onPress={() => setDayIndex(selectedIndex - 1)}
            />
          </View>
          <View style={{ flex: 1 }}>
            <Button
              title="Next day"
              secondary
              disabled={selectedIndex === scenario.days.length - 1}
              onPress={() => setDayIndex(selectedIndex + 1)}
            />
          </View>
        </View>
      ...
      {current.dataSource==='nessie' && current.developmentExpenseEnabled && <Card>
        <Text style={s.heading}>Explore an unexpected expense</Text>
        <Text style={s.body}>Add a $430 car repair to your test account and update your forecast.</Text>
        <Button title="Add $430 test expense" disabled={loading||!ready} onPress={()=>{
          setExpenseFailed(false);setExpenseMessage('Recording the expense and refreshing your forecast…');
          void simulateNessieExpense().then(setExpenseMessage).catch(()=>{setExpenseFailed(true);setExpenseMessage('The expense could not be confirmed. Refresh finances before retrying; it may already be pending.');});
        }}/>
        {expenseFailed && <Button title="Use sample finances" secondary disabled={loading} onPress={()=>void checkDemo(purchase,true).catch(()=>setExpenseMessage('Sample finances could not load. Please try again.'))}/>}
      </Card>}
      {expenseMessage?<Text accessibilityLiveRegion="polite" style={s.body}>{expenseMessage}</Text>:null}
      {current.dataSource === 'demo' && <Card tint={colors.soft}>
        <Text style={s.eyebrow}>What if?</Text>
        <Text style={s.heading}>Life changes. Your plan can, too.</Text>
        <Text style={s.body}>
          See how a $430 car repair changes your Safe Date.
        </Text>
        <Button
          title={repair ? "Remove car repair" : "Add unexpected expense"}
          icon="tool"
      ...
      <Text style={[s.body, { fontSize: 12 }]}>
        {current.dataSource === 'demo' ? 'Sample finances' : savedSnapshot ? 'Saved result' : 'Forecast'} · As of {dateLabel(profile.startDate, true)}.
      </Text>
      {current.change && <Card>
        <Text style={s.heading}>What changed?</Text>
